  ...
  return (
    <section className={`panel flex min-h-0 flex-col ${className}`}>
      <header className="flex shrink-0 items-center justify-between gap-2 border-b-2 border-fb-dim bg-black px-2 py-1.5">
        <h2 className={`pixel truncate text-[9px] sm:text-[10px] ${accentText[accent]}`}>{title}</h2>
        <div className="pixel shrink-0 whitespace-nowrap text-[7px] text-fb-ash sm:text-[8px]">
          {right}
        </div>
      </header>
